<template>
  <div class="todaycart">
    <div class="today" ref="todaywrrapper">
      <div>
        <!--{{checkfoods}}-->
        <ul class="">
          <transition name="fade" v-for="(food,index) in foodlists">
            <li class="list" v-if="food.Num>0" v-on:add="getNum" v-on:dec="getNum">
              ...
              <cartcontrol :foodlist="foodlists" :food="food" :checkfoods="checkfoods" :totalFoods="totalFoods"></cartcontrol>
            </li>
          </transition>
        </ul>
            ...
            <ul>
              <transition name="fade" v-for="(food,index) in invalidfoodlists">
                ...
                <li class="list" v-if="food.Num>0" v-on:add="getNum" v-on:dec="getNum">
                  ...
                  <cartcontrol :foodlist="foodlists" :food="food" :checkfoods="checkfoods"></cartcontrol>
    ...
    <div class="balance" v-if="foodlists.length>0">
      ...
      <span class="settle" @click.prevent.stop="toSureOrder">结算</span>
    </div>
    <transition name="fade">
...
export default {
  ...
  methods: {
    getToday: function() {
      ...
      this.$http.post(url, data).then((response) => {
        ...
        if (errnum == ERR_OK) {
          ...
          let foodsArr = [];
          validFoods.forEach(function(food) {
         ...
         this.$nextTick(() => {
            this.scroll = new BScroll(this.$refs.todaywrrapper, {
              click: false
            })
          })
      })
    ...
    deleteFood: function() {
      ...
      MessageBox.confirm(message, title, confirmButtonText).then(action => {
    ...
    toSureOrder: function() {
      ...
      if (this.invalidfoodlists.length > 0) {
        ...
        MessageBox.confirm(message, title).then(action => {
          ...
          //   })
          //   response = response.body;
          // bus.$emit('getAllFoods', response.Data);
