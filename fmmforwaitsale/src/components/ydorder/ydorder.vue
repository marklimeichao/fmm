  ...
  <div class="ydorder">
    <div class="ord-wrapper">
      ...
      <div class="order_bottom" v-if="!chooseShow">
        ...
        <button class="order_place" @click="toPay" :disabled="btnClick">立即下单</button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getList: function() {
      ...
      this.foodList.forEach((item, index) => {
        if (index < (this.foodList.length - 1)) {
          ...
          if (this.foodList.length == 1) {
            ...
            _this.foodsLists += "{'FoodId':'" + item.FoodId + "','Num':'" + item.Num + "','Days':'" + item.Days + "'}"
      ...
      //   var dList = res.Data.FoodList;

      //   this.foodsLists += '['
      //   dList.forEach(function(v) {
      //     if (dList.length == 1) {
      //       _this.foodsLists += "{'FoodId':'" + v.FoodId + "','Num':'" + v.Num + "','Days':'" + v.Zhou + "'}"
    ...
    getLocInfo: function(data) {
      ...
      // this.foodList=[];
      // this.foodIds=[];
      // console.log(data, this.chooseLocObj, this.foodList)
      let foodListStr = '';
      let chooseZhou = '';
      this.foodsLists = '';
      this.foodsLists += '['
    ...
    toPay: function() {
      ...
      let createOrderUrl = context + 'CreateYdOrder';
      let message = '您有商品库存不足，是否继续购买';
      let title = '';
      _this.btnClick = true;
      // console.log(this.foodsLists)
          ...
          MessageBox.confirm(message, title).then(action => {
            this.$http.post(createOrderUrl, getdatas).then((res) => {
              ...
              if (res.Code == 1) {
                ...
                // localStorage.setItem('ispay', "1");
                localStorage.removeItem('cartInfo');
                window.location.href = context + "cebbank/login.html?orderNo=" + no + "&type=2";
              } else {
                _this.btnClick = false;
                MessageBox.confirm('是否跳转到首页重新购买？', res.Msg).then(action => {
                  router.push('home')
