<template>
  <div class="invalid_order" ref="invalidwrapper">
    <div v-if="invalidOrder.length>0" class="invalidOrder">
            ...
            this.$nextTick(() => {
              if (!this.scroll) {
                ...
                this.scroll.on('touchEnd', (pos) => {
                  // 下拉动作
                  let invalidorder = document.querySelector('.invalidOrder').offsetHeight;
                  let inorder = document.querySelector('.invalid_order').offsetHeight;
                  // console.log(invalidorder)
                  // console.log(pos.y)
                  if (Math.abs(pos.y) - invalidorder + inorder > 50) {

                    this.getInvalidOrders()
                  }
                })
              } else {
                this.scroll.refresh()
        ...
        if (_this.ifscroll) {
          ...
          _this.$http.post(url, data).then((res) => {
            ...
            if (res.Data.length > 0) {
              ...
              _this.scroll = new BScroll(_this.$refs.invalidwrapper, {
                click: true,
                preventDefault: false
              })
              _this.loading = false;
